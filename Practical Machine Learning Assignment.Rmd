...
```{r}
missing_count = sapply(training_data, function(x) {sum(is.na(x))}) # Creates a vector of all column variabe and sums the number of NA values
```
```{r}
...
```{r}
sensor_related_Variable = grep(pattern = "_belt|_arm|_dumbbell|_forearm", names(predication_data))
```
```{r results = "hide"}
length(sensor_related_Variable) 
```
